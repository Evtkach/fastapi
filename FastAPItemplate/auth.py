...
from config import SMTP_USER, SMTP_PASSWORD, SMTP_SERVER, SMTP_PORT, RESET_PASSWORD_SECRET
...
def cleanup_expired_blacklisted_tokens() -> None:
    ...
    expired_tokens = [
        token for token,
        exp_time in blacklisted_tokens.items()
...
async def send_reset_email(email: str, token: str):
    # Ссылка, по которой пользователь перейдет на фронтенде
    reset_link = f"http://localhost:8000/auth/reset-password?token={token}"

    html = f"""
    <p>Здравствуйте!</p>
    <p>Вы запросили восстановление пароля. Пожалуйста, перейдите по ссылке ниже:</p>
    <a href="{reset_link}">Восстановить пароль</a>
...
def verify_reset_token(token: str) -> str | None:
    try:
        ...
        return payload.get("sub")  # Возвращает email пользователя
    except jwt.PyJWTError:
        return None  # Токен просрочен или изменен
